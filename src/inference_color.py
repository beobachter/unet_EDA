...
def inference_one(net, image, device):
    ...
    img = BasicDataset.preprocess(image, cfg.scale,1)
...
def get_args():
    ...
    parser.add_argument('--model', '-m', default='./data4/checkpoints/epoch_109.pth',
                        metavar='FILE',
                        help="Specify the file in which the model is stored")
    parser.add_argument('--input', '-i', dest='input', type=str, default='./data4/test/',
...
if __name__ == "__main__":
    ...
    for i, img_name in tqdm(enumerate(input_imgs)):
        ...
        if cfg.n_classes == 1:
            image_idx = Image.fromarray((mask * 120).astype(np.uint8))
